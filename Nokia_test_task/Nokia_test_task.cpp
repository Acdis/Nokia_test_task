...
std::vector<std::string> read_strings(std::string);
std::vector<std::string> separate_string(std::string);
std::string get_cell_value(std::string);

int calculate_formula(std::string);

bool is_char_eq(char);
bool is_digit(char);
bool is_sign(char);
...
int main(int argc, char* argv[] ) {
    ...
    //vector = read_strings(".../Nokia_test_task/1.csv");
...
std::string get_cell_value(std::string address) {
    int i = 0;

    while (!is_digit(address[i]))
        i++;

    std::string column = address.substr(0, i);
    std::string line = address.substr(i, address.npos);
